...
class Heart:
    ...
    def calc_position(x, y, ratio):
        ...
        dx = ratio * force * (x - CANVAS_CENTER_X) + random.uniform(-0.5, 0.5)
        dy = ratio * force * (y - CANVAS_CENTER_Y) + random.uniform(-0.5, 0.5)
        return x - dx, y - dy

    def calc(self, generate_frame):
        ratio = 18 * curve(generate_frame / 28 * pi)
        halo_radius = int(4 + 6 * (1 + curve(generate_frame / 10 * pi)))
        halo_number = int(2000 + 4000 * abs(curve(generate_frame / 10 * pi) ** 2))
        all_points = []
